...
- Android：`AILearningOS-android-v1.7.1.apk`
- 应用显示名：`AILearningOS`
- Android 包名：`com.georgegao.ai_study_os`（为覆盖旧版安装而保留）
- 版本：`1.7.1`，versionCode `25`
- 构建产物及 SHA-256：由 `v1.7.1` GitHub Release 的 `SHA256SUMS.txt` 给出
- 最新后端：`AILearningOS-server-windows-x64-v4.4.zip`
- 后端源码：`AILearningOS-server-source-v4.4.zip`
- ChatGPT 登录：保留 v3.9 的 durable binding 与恢复机制；每个 LearningOS 用户仍只有自己的隔离 Codex 登录态
- Canvas：课程作业成为主页待办信息源，并按提交状态分到“今日待办”与“已完成”
- AI 路由：默认手动选择；用户显式开启后，Codex 额度用尽才临时走 DeepSeek。后端会等所有已耗尽的官方限额窗口恢复后才自动切回，避免反复失败。
