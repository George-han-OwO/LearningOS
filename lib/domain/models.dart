...
class AiConnectionSettings {
  ...
  bool get ready => enabled && (apiKeyConfigured || apiKey.trim().isNotEmpty);

  /// True only while a newly typed key is present in client memory. Local AI
  /// services must use this instead of [ready] so they never send an empty key.
  bool get hasClientApiKey => enabled && apiKey.trim().isNotEmpty;
...
class PendingWordEnrichmentState {
  const PendingWordEnrichmentState({
  ...
  static const idle = PendingWordEnrichmentState(
    status: 'idle',
    pendingCount: 0,
    requestedCount: 0,
    enrichedCount: 0,
    message: '没有待 AI 补全的词条。',
  ...
  bool get waitingForApiKey =>
      status == 'waiting_for_api_key' || status == 'waiting_for_server_key';
  bool get retrying => status == 'retrying';
  bool get working => status == 'checking' || status == 'busy';

  factory PendingWordEnrichmentState.fromMap(Map<String, Object?> map) {
    return PendingWordEnrichmentState(
      ...
      enrichedCount: (map['enriched_count'] as num?)?.toInt() ?? 0,
      message: map['message']?.toString() ?? '',
      retryIntervalSeconds:
          (map['retry_interval_seconds'] as num?)?.toInt() ?? 30,
...
class ConversationImportResult {
  const ConversationImportResult({
    required this.title,
...
class CourseQuestion {
  ...
  // The server omits this field from normal chapter responses.
...
class StudyWord {
  ...
  bool get needsAiEnrichment =>
      ...
      exampleEnglish.trim().isEmpty ||
      exampleChinese.trim().isEmpty;
...
class TodayStats {
  ...
  factory TodayStats.fromMap(Map<String, Object?> map) {
    final checked = map['checked_in'];
    final checkedIn =
        checked == true ||
        (checked is num && checked.toInt() == 1) ||
        (checked is String && checked.trim().toLowerCase() == 'true');
    return TodayStats(
      dueWords: (map['due_words'] as num?)?.toInt() ?? 0,
      newWords: (map['new_words'] as num?)?.toInt() ?? 0,
      reviewsCompleted: (map['reviews_completed'] as num?)?.toInt() ?? 0,
